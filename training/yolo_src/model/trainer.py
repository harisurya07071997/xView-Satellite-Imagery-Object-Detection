...
# aws configure
# wandb login <>
# s5cmd cp "s3://aws-hari-ml/xView_Satellite_Imagery_OD/v1/*" v1/
# rm -rf ~/.aws
...
def main(args: argparse.Namespace, config: dict) -> None:
    ...
    if args.do_tuning:
        try:
            ...
            config["model"]["yolo"]["copy_paste" ] = wandb_run.config.get("copy_paste" , config["model"]["yolo"]["copy_paste" ])
            config["model"]["yolo"]["cutmix"] = wandb_run.config.get("cutmix", config["model"]["yolo"]["cutmix"])
            config["model"]["yolo"]["mosaic"] = wandb_run.config.get("mosaic", config["model"]["yolo"]["mosaic"])

            config["model"]["yolo"]["degrees"] = wandb_run.config.get("degrees", config["model"]["yolo"]["degrees"])

            config["model"]["yolo"]["flipud"] = wandb_run.config.get("flipud", config["model"]["yolo"]["flipud"])
            config["model"]["yolo"]["fliplr"] = wandb_run.config.get("fliplr", config["model"]["yolo"]["fliplr"])

            config["model"]["yolo"]["cls_pw"] = wandb_run.config.get("cls_pw", config["model"]["yolo"]["cls_pw"])
